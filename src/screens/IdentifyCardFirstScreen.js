...
export default class IdentifyCardFirstScreen extends Component {
  ...
  onPressRightBarButton() {
    ...
    this.props.detectId(formData)
      .then(response => {
        ...
        const data = _.get(response, 'data.data.document.data', null)
        const img_url = _.get(response, 'data.data.img_url', null)
        const dataPass = { ...data, front: img_url }
        if (!data) {
          this.showAlert(i18next.t('ErrorImage'), i18next.t('TryAgain'))
          HUD.hideLoading()
          return
        }
        pushScreenBackID(this.props.componentId, dataPass)
        HUD.hideLoading()
      })
      .catch(error => {
        this.showAlert(JSON.stringify('error'), i18next.t('TryAgain'))
        console.log(error);
        HUD.hideLoading()
      })
  }
    ...
    return (
      <ImageBackground source={Images.background} style={styles.container}>
        ...
        <SafeAreaView style={[styles.container]}>
          <Text style={{ marginTop: 64, alignSelf: 'center', fontSize: 22 }}>{i18next.t('UploadFrontIdCard')}</Text>
          <TouchableOpacity
